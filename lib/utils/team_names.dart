import '../services/i18n.dart';
import '../services/team_overrides.dart';
import 'team_names_i18n.g.dart' as gen;

/// 队名本地化 — 仅 zh 提供翻译,其他语言保留原英文(足球语境英文通用)。
///
/// 数据范围:5 大欧洲联赛(英超/西甲/意甲/德甲/法甲)的现有 96 支球队 + 常见
/// 备用名。新球队进入 feed 时(转会/升降级)直接显示英文,补到这张表即可。
const Map<String, String> _zh = {
  // England - Premier League
  'AFC Bournemouth': '伯恩茅斯',
  'Arsenal FC': '阿森纳',
  'Aston Villa': '阿斯顿维拉',
  'Brentford FC': '布伦特福德',
  'Brighton & Hove Albion': '布莱顿',
  'Burnley FC': '伯恩利',
  'Chelsea FC': '切尔西',
  'Crystal Palace': '水晶宫',
  'Everton FC': '埃弗顿',
  'Fulham FC': '富勒姆',
  'Leeds United': '利兹联',
  'Liverpool FC': '利物浦',
  'Manchester City': '曼城',
  'Manchester United': '曼联',
  'Newcastle United': '纽卡斯尔',
  'Nottingham Forest': '诺丁汉森林',
  'Sunderland AFC': '桑德兰',
  'Tottenham Hotspur': '热刺',
  'West Ham United': '西汉姆联',
  'Wolverhampton Wanderers': '狼队',

  // Spain - LaLiga
  'Athletic Bilbao': '毕尔巴鄂竞技',
  'Atletico Madrid': '马德里竞技',
  'CA Osasuna': '奥萨苏纳',
  'Deportivo Alaves': '阿拉维斯',
  'Elche CF': '埃尔切',
  'Espanyol Barcelona': '西班牙人',
  'FC Barcelona': '巴塞罗那',
  'Getafe CF': '赫塔费',
  'Girona FC': '赫罗纳',
  'Levante UD': '莱万特',
  'RC Celta de Vigo': '塞尔塔',
  'RCD Mallorca': '马洛卡',
  'Rayo Vallecano': '巴列卡诺',
  'Real Betis Seville': '皇家贝蒂斯',
  'Real Madrid': '皇家马德里',
  'Real Oviedo': '皇家奥维耶多',
  'Real Sociedad San Sebastian': '皇家社会',
  'Sevilla FC': '塞维利亚',
  'Valencia CF': '瓦伦西亚',
  'Villarreal CF': '比利亚雷亚尔',

  // Italy - Serie A
  'AC Milan': 'AC 米兰',
  'ACF Fiorentina': '佛罗伦萨',
  'AS Roma': '罗马',
  'Atalanta BC': '亚特兰大',
  'Bologna FC': '博洛尼亚',
  'Cagliari Calcio': '卡利亚里',
  'Como 1907': '科莫',
  'Genoa CFC': '热那亚',
  'Hellas Verona': '维罗纳',
  'Inter Milano': '国际米兰',
  'Juventus Turin': '尤文图斯',
  'Lazio Rome': '拉齐奥',
  'Parma Calcio': '帕尔马',
  'Pisa SC': '比萨',
  'SSC Napoli': '那不勒斯',
  'Sassuolo Calcio': '萨索洛',
  'Torino FC': '都灵',
  'US Cremonese': '克雷莫纳',
  'US Lecce': '莱切',
  'Udinese Calcio': '乌迪内斯',

  // Germany - Bundesliga
  '1. FC Cologne': '科隆',
  '1. FC Heidenheim': '海登海姆',
  'Bayer Leverkusen': '勒沃库森',
  'Bayern Munich': '拜仁慕尼黑',
  'Borussia Dortmund': '多特蒙德',
  'Borussia Monchengladbach': '门兴格拉德巴赫',
  'Eintracht Frankfurt': '法兰克福',
  'FC Augsburg': '奥格斯堡',
  'FC St. Pauli': '圣保利',
  'FSV Mainz': '美因茨',
  'Hamburger SV': '汉堡',
  'RB Leipzig': '莱比锡红牛',
  'SC Freiburg': '弗赖堡',
  'TSG Hoffenheim': '霍芬海姆',
  'Union Berlin': '柏林联合',
  'VFL Wolfsburg': '沃尔夫斯堡',
  'VfB Stuttgart': '斯图加特',
  'Werder Bremen': '不来梅',

  // China - Chinese Super League (16 teams)
  'Beijing Guoan': '北京国安',
  'Chengdu Rongcheng': '成都蓉城',
  'Chongqing Tonglianglong FC': '重庆铜梁龙',
  'Dalian Yingbo FC': '大连英博',
  'Henan': '河南',
  'Liaoning Tieren FC': '辽宁铁人',
  'Qingdao Hainiu FC': '青岛海牛',
  'Qingdao West Coast FC': '青岛西海岸',
  'Shandong Taishan FC': '山东泰山',
  'Shanghai Port FC': '上海海港',
  'Shanghai Shenhua FC': '上海申花',
  'Shenzhen Peng City': '深圳新鹏城',
  'Tianjin Jinmen Tiger': '天津津门虎',
  'Wuhan Three Towns FC': '武汉三镇',
  'Yunnan Yukun': '云南玉昆',
  'Zhejiang FC': '浙江',

  // China - China League 1 (中甲 — 部分)
  'Dalian Kun City': '大连鲲城',
  'Foshan Nanshi FC': '佛山南狮',
  'Guandong GZ-Power FC': '广州队',
  'Guangxi Hengchen FC': '广西恒晨',
  'Nantong Zhiyun': '南通支云',
  'Ningbo Professional FC': '宁波职业',
  'Shaanxi Union FC': '陕西联合',
  'Shenzhen Juniors FC': '深圳青年人',
  'Shijiazhuang Gongfu FC': '石家庄功夫',
  'Suzhou Dongwu': '苏州东吴',
  'Wuxi Wugou': '无锡吴钩',
  'Yanbian Longding': '延边龙鼎',

  // Japan - J.League (J1)
  'Avispa Fukuoka': '福冈黄蜂',
  'Cerezo Osaka': '大阪樱花',
  'FC Tokyo': '东京FC',
  'Fagiano Okayama': '冈山雉鸡',
  'Gamba Osaka': '大阪钢巴',
  'JEF United Chiba': '千叶市原',
  'Kashima Antlers': '鹿岛鹿角',
  'Kashiwa Reysol': '柏太阳神',
  'Kawasaki Frontale': '川崎前锋',
  'Kyoto Sanga FC': '京都不死鸟',
  'Machida Zelvia': '町田泽维亚',
  'Mito Hollyhock': '水户蜀葵',
  'Nagoya Grampus': '名古屋鲸鱼',
  'Sanfrecce Hiroshima': '广岛三箭',
  'Shimizu S-Pulse': '清水心跳',
  'Tokyo Verdy': '东京绿茵',
  'Urawa Red Diamonds': '浦和红钻',
  'V-Varen Nagasaki': '长崎成功丸',
  'Vissel Kobe': '神户胜利船',
  'Yokohama F Marinos': '横滨水手',

  // Japan - J.League 2 (J2 — 主要球队)
  'Albirex Niigata': '新泻天鹅',
  'Blaublitz Akita': '秋田蓝闪电',
  'Hokkaido Consadole Sapporo': '札幌冈萨多',
  'Iwaki FC': '磐城',
  'Jubilo Iwata': '磐田喜悦',
  'Montedio Yamagata': '山形山神',
  'Oita Trinita': '大分三神',
  'RB Omiya Ardija': '大宫松鼠',
  'Roasso Kumamoto': '熊本深红',
  'Sagan Tosu': '鸟栖砂岩',
  'Shonan Bellmare': '湘南海洋',
  'Tochigi SC': '枥木',
  'Tokushima Vortis': '德岛漩涡',
  'Vegalta Sendai': '仙台维加泰',
  'Ventforet Kofu': '甲府风林',
  'Yokohama FC': '横滨FC',
  'Zweigen Kanazawa': '金泽',

  // Republic of Korea - K-League 1 (韩K1)
  'Bucheon FC 1995': '富川',
  'Daejeon Citizen FC': '大田市民',
  'FC Anyang': '安养',
  'FC Pohang Steelers': '浦项制铁',
  'FC Seoul': '首尔FC',
  'Gangwon FC': '江原',
  'Gimcheon Sangmu FC': '金泉尚武',
  'Gwangju FC': '光州',
  'Incheon United FC': '仁川联',
  'Jeju SK FC': '济州联',
  'Jeonbuk FC': '全北现代',
  'Ulsan HD FC': '蔚山现代',

  // Saudi Arabia - Saudi Pro League (沙特联)
  'Al Ahli Saudi FC': '吉达国民',
  'Al Hilal SFC': '利雅得新月',
  'Al Nassr Club': '利雅得胜利',
  'Al-Fateh SC': '法塔赫',
  'Al-Fayha FC': '法伊哈',
  'Al-Ittifaq FC': '伊蒂法克',
  'Al-Ittihad Club': '吉达联合',
  'Al-Khaleej Club': '哈利杰',
  'Al-Kholood': '霍卢德',
  'Al-Najma': '纳吉马',
  'Al-Riyadh SC': '利雅得',
  'Al-Shabab FC (SA)': '青年人(沙特)',
  'Al-Taawoun FC': '塔阿文',
  'Al-Qadsiah FC': '卡迪西亚',

  // France - Ligue 1
  'AJ Auxerre': '欧塞尔',
  'AS Monaco': '摩纳哥',
  'Angers SCO': '昂热',
  'FC Lorient': '洛里昂',
  'FC Metz': '梅斯',
  'FC Nantes': '南特',
  'Le Havre AC': '勒阿弗尔',
  'Lille OSC': '里尔',
  'OGC Nice': '尼斯',
  'Olympique Lyon': '里昂',
  'Olympique Marseille': '马赛',
  'Paris FC': '巴黎 FC',
  'Paris Saint-Germain': '巴黎圣日耳曼',
  'Racing Club De Lens': '朗斯',
  'Stade Brest 29': '布雷斯特',
  'Stade Rennais FC': '雷恩',
  'Strasbourg Alsace': '斯特拉斯堡',
  'Toulouse FC': '图卢兹',

  // Netherlands - Eredivisie (荷甲)
  'AZ Alkmaar': '阿尔克马尔',
  'Ajax Amsterdam': '阿贾克斯',
  'Almere City FC': '阿尔默勒城',
  'De Graafschap': '格拉夫沙普',
  'Excelsior Rotterdam': '鹿特丹精英',
  'FC Groningen': '格罗宁根',
  'FC Twente Enschede': '特温特',
  'FC Utrecht': '乌得勒支',
  'FC Volendam': '福伦丹',
  'Feyenoord Rotterdam': '费耶诺德',
  'Fortuna Sittard': '锡塔德',
  'Go Ahead Eagles': '前进之鹰',
  'Heracles Almelo': '赫拉克勒斯',
  'NAC Breda': '布雷达',
  'NEC Nijmegen': '奈梅亨',
  'PEC Zwolle': '兹沃勒',
  'PSV Eindhoven': '埃因霍温',
  'RKC Waalwijk': '瓦尔韦克',
  'SC Heerenveen': '海伦芬',
  'SC Telstar': '特尔斯达',
  'Sparta Rotterdam': '斯巴达鹿特丹',
  'Willem II Tilburg': '蒂尔堡威廉二世',

  // Portugal - Liga Portugal (葡超)
  'AVS Futebol SAD': 'AVS',
  'CD Tondela': '通德拉',
  'Casa Pia Lisbon': '卡萨皮亚',
  'Estoril Praia': '埃斯托里尔',
  'Estrela Amadora': '阿马多拉之星',
  'FC Alverca SAD': '阿尔韦卡',
  'FC Arouca': '阿罗卡',
  'FC Famalicao': '法马利康',
  'FC Porto': '波尔图',
  'Gil Vicente Barcelos': '吉尔维森特',
  'Moreirense FC': '莫雷伦斯',
  'Nacional da Madeira': '马德拉国民',
  'Rio Ave FC': '里奥阿维',
  'SC Braga': '布拉加',
  'SL Benfica': '本菲卡',
  'Santa Clara Azores': '圣克拉拉',
  'Sporting CP': '葡萄牙体育',
  'Vitoria SC Guimaraes': '吉马良斯',

  // Turkiye - Super Lig (土超)
  'Alanyaspor': '阿兰亚体育',
  'Antalyaspor': '安塔利亚体育',
  'Besiktas Istanbul': '贝西克塔斯',
  'Caykur Rizespor': '里泽体育',
  'Eyupspor': '艾郁普体育',
  'Fatih Karagumruk Istanbul': '法蒂赫卡拉古卢克',
  'Fenerbahce Istanbul': '费内巴切',
  'Galatasaray Istanbul': '加拉塔萨雷',
  'Gaziantep FK': '加济安泰普',
  'Genclerbirligi SK': '青年联盟',
  'Goztepe Izmir': '伊兹密尔哥兹特佩',
  'Istanbul Basaksehir': '伊斯坦布尔巴萨克塞希尔',
  'Kasimpasa Istanbul': '卡斯帕萨',
  'Kayserispor': '开塞利体育',
  'Kocaelispor': '科贾埃利体育',
  'Konyaspor': '科尼亚体育',
  'Samsunspor': '萨姆松体育',
  'Trabzonspor': '特拉布宗体育',

  // Scotland - Premiership (苏超)
  'Aberdeen FC': '阿伯丁',
  'Arbroath FC': '阿布罗斯',
  'Celtic Glasgow': '凯尔特人',
  'Dundee FC': '邓迪',
  'Dundee United': '邓迪联',
  'Dunfermline Athletic FC': '邓弗姆林',
  'Falkirk FC': '法尔柯克',
  'Glasgow Rangers': '流浪者',
  'Heart of Midlothian FC': '哈茨',
  'Hibernian FC': '希伯尼安',
  'Kilmarnock FC': '基尔马诺克',
  'Livingston FC': '利文斯顿',
  'Motherwell FC': '马瑟韦尔',
  'St Mirren FC': '圣米伦',

  // Belgium - Pro League (比甲)
  'Cercle Brugge': '布鲁日竞技',
  'Club Brugge': '布鲁日',
  'FCV Dender EH': '登德',
  'K Beerschot VA': '比尔斯霍特',
  'KAA Gent': '亨克根特',
  'KRC Genk': '亨克',
  'KVC Westerlo': '韦斯特洛',
  'Lommel SK': '洛梅尔',
  'Oud-Heverlee Leuven': '鲁汶',
  'RAAL La Louviere': '拉鲁维埃',
  'RSC Anderlecht': '安德莱赫特',
  'Royal Antwerp FC': '皇家安特卫普',
  'Royal Charleroi SC': '沙勒罗瓦',
  'SV Zulte Waregem': '泽尔特瓦雷赫姆',
  'St. Truidense VV': '圣特鲁登',
  'Standard Liege': '标准列日',
  'Union Saint-Gilloise': '圣吉罗斯',
  'Yellow-Red KV Mechelen': '梅赫伦',

  // Switzerland - Super League (瑞超)
  'FC Basel 1893': '巴塞尔',
  'FC Lugano': '卢加诺',
  'FC Luzern': '卢塞恩',
  'FC Sion': '锡永',
  'FC St. Gallen 1879': '圣加仑',
  'FC Thun': '图恩',
  'FC Winterthur': '温特图尔',
  'FC Zurich': '苏黎世',
  'Grasshopper Club Zurich': '草蜢',
  'Lausanne-Sport': '洛桑体育',
  'Servette Geneva': '塞尔维特',
  'Young Boys Bern': '伯尔尼年轻人',

  // Austria - Bundesliga (奥甲)
  'Blau-Weiss Linz': '林茨蓝白',
  'FC Salzburg': '萨尔茨堡红牛',
  'FK Austria Wien': '奥地利维也纳',
  'Grazer AK 1902': '格拉茨',
  'LASK Linz': '林茨',
  'SCR Altach': '阿尔塔赫',
  'SK Rapid': '维也纳快速',
  'SK Sturm Graz': '格拉茨风暴',
  'SV Ried': '里德',
  'TSV Hartberg': '哈特贝格',
  'WSG Tirol': '蒂罗尔',
  'Wolfsberger AC': '沃尔夫斯堡格',

  // Denmark - Superliga (丹麦超)
  'AGF Aarhus': '奥胡斯',
  'Broendby IF': '布隆德比',
  'FC Copenhagen': '哥本哈根',
  'FC Fredericia': '弗雷德里西亚',
  'FC Midtjylland': '中日德兰',
  'FC Nordsjaelland': '北西兰岛',
  'Odense Boldklub': '欧登塞',
  'Randers FC': '兰德斯',
  'Silkeborg IF': '锡尔克堡',
  'Soenderjyske': '南日德兰',
  'Vejle BK': '瓦埃勒',
  'Viborg FF': '维堡',

  // Sweden - Allsvenskan (瑞典超)
  'AIK': 'AIK 索尔纳',
  'BK Hacken': '哈肯',
  'Degerfors IF': '迪格福斯',
  'Djurgardens IF': '尤尔加登',
  'GAIS': 'GAIS',
  'Halmstads BK': '哈尔姆斯塔德',
  'Hammarby IF': '哈马比',
  'IF Brommapojkarna': '布罗马波卡纳',
  'IF Elfsborg': '埃尔夫斯堡',
  'IFK Goteborg': '哥德堡',
  'IK Sirius': '西里乌斯',
  'Kalmar FF': '卡尔马',
  'Malmo FF': '马尔默',
  'Mjallby AIF': '米亚尔比',
  'Orgryte IS': '奥格里特',
  'Vasteraas SK': '韦斯特罗斯',

  // Norway - Eliteserien (挪威超)
  'Aalesunds FK': '阿勒松',
  'Bodoe/Glimt': '博多/格林特',
  'Fredrikstad FK': '腓特烈斯塔',
  'HamKam': '哈玛',
  'IK Start': '斯塔特',
  'KFUM Oslo': '奥斯陆KFUM',
  'Kristiansund BK': '克里斯蒂安松',
  'Lillestroem SK': '利勒斯特罗姆',
  'Molde FK': '莫尔德',
  'Rosenborg BK': '罗森博格',
  'SK Brann': '布兰',
  'Sandefjord Fotball': '桑德菲尤德',
  'Sarpsborg 08': '萨普斯堡',
  'Tromsoe IL': '特罗姆瑟',
  'Vaalerenga IF': '瓦勒伦加',
  'Viking FK': '维京',

  // Poland - Ekstraklasa (波兰甲)
  'Bruk-Bet Termalica Nieciecza': '尼塞恰',
  'GKS Katowice': '卡托维兹',
  'GKS Piast Gliwice': '皮亚斯特',
  'Gornik Zabrze': '扎布热戈尔尼克',
  'Jagiellonia Bialystok': '比亚韦斯托克',
  'KKS Lech Poznan': '波兹南莱赫',
  'KS Lechia Gdansk': '格但斯克',
  'Korona Kielce': '凯尔采',
  'LKP Motor Lublin': '卢布林汽车',
  'Legia Warszawa': '华沙莱吉亚',
  'MKS Arka Gdynia': '格丁尼亚',
  'MKS Cracovia Krakow': '克拉科维亚',
  'Pogon Szczecin': '什切青',
  'RKS Radomiak Radom': '拉多米雅克',
  'RKS Rakow Czestochowa': '琴斯托霍瓦',
  'Widzew Lodz': '罗兹',
  'Wisla Plock': '普沃茨克',
  'Zaglebie Lubin': '卢宾',

  // Greece - Super League (希腊超)
  'AE Kifisia FC': '基菲西亚',
  'AE Larissa FC': '拉里萨',
  'AEK Athens': 'AEK 雅典',
  'APO Levadiakos FC': '莱瓦迪亚科斯',
  'Aris Thessaloniki': '塞萨洛尼基阿利斯',
  'Asteras Tripolis': '特里波利斯之星',
  'Atromitos Athinon': '雅典阿特罗米托斯',
  'OFI Crete': '克里特OFI',
  'Olympiacos Piraeus': '奥林匹亚科斯',
  'PAOK Thessaloniki': '塞萨洛尼基PAOK',
  'Panaitolikos Agrinio': '阿格里尼奥',
  'Panathinaikos Athens': '雅典帕纳辛奈科斯',
  'Panserraikos FC': '潘塞拉伊科斯',
  'Volos NPS': '沃洛斯',

  // Russia - Premier League (俄超)
  'CSKA Moscow': '莫斯科中央陆军',
  'FC Baltika Kaliningrad': '波罗的海',
  'FC Dynamo-Makhachkala': '马哈奇卡拉迪纳摩',
  'FC Orenburg': '奥伦堡',
  'FC Pari Nizhny Novgorod': '下诺夫哥罗德',
  'FK Akron Tolyatti': '阿克伦',
  'FK Dinamo Moscow': '莫斯科迪纳摩',
  'FK Krasnodar': '克拉斯诺达尔',
  'FK Rostov': '罗斯托夫',
  'FK Rubin Kazan': '喀山红宝石',
  'FK Sochi': '索契',
  'FK Spartak Moscow': '莫斯科斯巴达',
  'FK Zenit Saint Petersburg': '泽尼特',
  'Lokomotiv Moscow': '莫斯科火车头',
  'PFK Krylia Sovetov Samara': '萨马拉苏维埃之翼',
  'RFK Akhmat Grozny': '艾哈迈特',

  // Ukraine - Premier League (乌超)
  'FC Dynamo Kyiv': '基辅迪纳摩',
  'FC Epitsentr Kamianets-Podilskyi': '埃皮岑特尔',
  'FC Kryvbas Kriviy Rih': '克里夫巴斯',
  'FC Metalist 1925 Kharkiv': '哈尔科夫金属',
  'FC Obolon Kyiv': '基辅奥博隆',
  'FC Oleksandriya': '亚历山大里亚',
  'FC Polissya Zhytomyr': '波利西亚',
  'FC Rukh Lviv': '利沃夫鲁克',
  'FC Shakhtar Donetsk': '顿涅茨克矿工',
  'FC Zorya Luhansk': '卢甘斯克曙光',
  'FK Kudrivka': '库德利夫卡',
  'Karpaty Lviv': '利沃夫喀尔巴阡',
  'Kolos Kovalivka': '科洛斯',
  'LNZ Cherkasy': '切尔卡瑟',
  'SC Poltava': '波尔塔瓦',
  'Veres Rivne': '里夫内',

  // Czechia - 1. Liga (捷甲) — 先覆盖,等加上联赛后启用
  'AC Sparta Prague': '布拉格斯巴达',
  'SK Slavia Prague': '布拉格斯拉维亚',
  'FC Viktoria Plzen': '比尔森胜利',

  // Saudi Arabia - Saudi Pro League (沙特联) — 补缺
  'Al-Hazm': '哈兹姆',
  'Al-Okhdood Club': '欧胡杜德',
  'Damac FC': '达马克',
  'Neom SC': '尼奥姆',

  // Thailand - Thai League 1 (泰超)
  'Ayutthaya United FC': '阿育他亚联',
  'BG Pathum United': 'BG巴吞联',
  'Bangkok United FC': '曼谷联',
  'Buriram United': '武里南联',
  'Chiangrai United': '清莱联',
  'Chonburi FC': '春武里',
  'Kanchanaburi Power FC': '北碧动力',
  'Lamphun Warriors FC': '南奔战士',
  'Muang Thong United': '曼通联',
  'Nakhon Ratchasima FC': '呵叻',
  'Port FC': '港口',
  'Prachuap FC': '巴蜀',
  'Ratchaburi FC': '叻丕',
  'Rayong FC': '罗勇',
  'Sukhothai FC': '素可泰',
  'Uthai Thani FC': '乌泰他尼',

  // USA - MLS (美职)
  'Atlanta United FC': '亚特兰大联',
  'Austin FC': '奥斯汀',
  'CF Montreal': '蒙特利尔',
  'Charlotte FC': '夏洛特',
  'Chicago Fire': '芝加哥火焰',
  'Colorado Rapids': '科罗拉多急流',
  'Columbus Crew': '哥伦布机员',
  'DC United': '华盛顿联',
  'FC Cincinnati': '辛辛那提',
  'FC Dallas': '达拉斯',
  'Houston Dynamo': '休斯顿迪纳摩',
  'Inter Miami CF': '迈阿密国际',
  'Los Angeles FC': '洛杉矶FC',
  'Los Angeles Galaxy': '洛杉矶银河',
  'Minnesota United FC': '明尼苏达联',
  'Nashville SC': '纳什维尔',
  'New England Revolution': '新英格兰革命',
  'New York City FC': '纽约城',
  'New York Red Bulls': '纽约红牛',
  'Orlando City SC': '奥兰多城',
  'Philadelphia Union': '费城联',
  'Portland Timbers': '波特兰伐木者',
  'Real Salt Lake': '盐湖城皇家',
  'Saint Louis City SC': '圣路易斯城',
  'San Diego FC': '圣迭戈',
  'San Jose Earthquakes': '圣何塞地震',
  'Seattle Sounders': '西雅图海湾人',
  'Sporting Kansas City': '堪萨斯城竞技',
  'Toronto FC': '多伦多',
  'Vancouver Whitecaps FC': '温哥华白浪',

  // Mexico - Liga MX (墨西哥甲)
  'Atlas FC': '阿特拉斯',
  'CD Guadalajara': '瓜达拉哈拉',
  'CF America': '美洲狮',
  'CF Cruz Azul': '蓝十字',
  'CF Pachuca': '帕丘卡',
  'Deportivo Toluca FC': '托卢卡',
  'Pumas UNAM': '普马斯',
  'Tigres UANL': '老虎',
  'Club Tijuana': '蒂华纳',
  'Monterrey': '蒙特雷',
  'CF Monterrey': '蒙特雷',
  'Necaxa': '内卡萨',
  'CF Pachuca - Tuzos': '帕丘卡',
  'Club Leon': '莱昂',
  'Mazatlan FC': '马萨特兰',
  'Puebla FC': '普埃布拉',
  'Queretaro FC': '克雷塔罗',
  'Santos Laguna': '桑托斯拉古纳',

  // Argentina - Liga Profesional (阿甲)
  'Boca Juniors': '博卡青年',
  'River Plate': '河床',
  'Independiente': '独立',
  'Racing Club': '竞技',
  'Club Atletico San Lorenzo': '圣洛伦索',
  'San Lorenzo': '圣洛伦索',
  'CA Velez Sarsfield': '萨斯菲尔德',
  'Velez Sarsfield': '萨斯菲尔德',
  'Huracan': '飓风',
  'Estudiantes La Plata': '拉普拉塔大学生',
  'Estudiantes de La Plata': '拉普拉塔大学生',
  'CA Lanus': '拉努斯',
  'Lanus': '拉努斯',
  'Argentinos Juniors': '阿根廷青年',
  'CA Talleres Cordoba': '塔列雷斯',
  'Talleres Cordoba': '塔列雷斯',
  'Banfield': '班菲尔德',
  'Defensa y Justicia': '正义之剑',
  'Newell\'s Old Boys': '老男孩',
  'Rosario Central': '罗萨里奥中央',
  'Gimnasia La Plata': '拉普拉塔体操',
  'Union Santa Fe': '圣菲联合',
  'Atletico Tucuman': '图库曼竞技',
  'Belgrano Cordoba': '科尔多瓦贝尔格拉诺',
  'CA Belgrano': '科尔多瓦贝尔格拉诺',
  'Godoy Cruz': '戈多伊克鲁斯',
  'CA Godoy Cruz Antonio Tomba': '戈多伊克鲁斯',
  'Instituto AC Cordoba': '科尔多瓦学院',
  'Estudiantes Rio Cuarto': '里奥夸尔托大学生',
  'Estudiantes de Rio Cuarto': '里奥夸尔托大学生',
  'CA Platense': '普拉滕斯',
  'Platense': '普拉滕斯',
  'CA Tigre': '猛虎',
  'Tigre': '猛虎',
  'CA Barracas Central': '巴拉卡斯中央',
  'Barracas Central': '巴拉卡斯中央',
  'CA Aldosivi': '阿尔多西维',
  'Aldosivi': '阿尔多西维',
  'CA Sarmiento de Junin': '萨米恩托',
  'Sarmiento Junin': '萨米恩托',
  'Central Cordoba': '科尔多瓦中央',
  'CA Central Cordoba SdE': '科尔多瓦中央',
  'CA Independiente Rivadavia': '里瓦达维亚独立',
  'Independiente Rivadavia': '里瓦达维亚独立',
  'CA Deportivo Riestra': '里斯特拉',
  'Deportivo Riestra': '里斯特拉',

  // Brazil - Brasileiro Serie A (巴甲)
  'Atletico Mineiro MG': '米内罗竞技',
  'Botafogo FR RJ': '博塔弗戈',
  'CA Paranaense PR': '帕拉纳竞技',
  'CR Flamengo RJ': '弗拉门戈',
  'CR Vasco da Gama RJ': '瓦斯科达伽马',
  'Chapecoense SC': '查皮科恩斯',
  'Clube do Remo PA': '雷莫',
  'Coritiba FC PR': '科里蒂巴',
  'Cruzeiro EC MG': '克鲁塞罗',
  'EC Bahia BA': '巴伊亚',
  'EC Vitoria BA': '维多利亚',
  'Fluminense FC RJ': '弗鲁米嫩塞',
  'Gremio FB Porto Alegrense RS': '格雷米奥',
  'Mirassol FC SP': '米拉索尔',
  'Red Bull Bragantino SP': '红牛布拉干蒂诺',
  'SC Corinthians SP': '科林蒂安',
  'SC Internacional RS': '国际队',
  'SE Palmeiras SP': '帕梅拉斯',
  'Santos FC SP': '桑托斯',
  'Sao Paulo FC SP': '圣保罗',
  'Ceara SC CE': '塞阿拉',
  'EC Juventude RS': '尤文图德',
  'Sport Recife PE': '累西腓体育',
  'Fortaleza EC CE': '福塔雷萨',

  // ── 2026-05-06 补:8 联赛 82 队(prod feed 实查未翻译)─────────────────
  // Spain - LaLiga 2(西乙)
  'AD Ceuta': '休达',
  'Albacete Balompie': '阿尔巴塞特',
  'Burgos CF': '布尔戈斯',
  'CD Castellon': '卡斯特利翁',
  'CD Leganes': '莱加内斯',
  'CD Mirandes': '米兰德斯',
  'Cadiz CF': '加的斯',
  'Cordoba CF': '科尔多瓦',
  'Cultural Leonesa': '莱昂文化',
  'FC Andorra': '安道尔',
  'Granada CF': '格拉纳达',
  'RC Deportivo De La Coruna': '拉科鲁尼亚',
  'Racing Santander': '桑坦德竞技',
  'SD Eibar': '埃瓦尔',
  'UD Almeria': '阿尔梅里亚',
  'UD Las Palmas': '拉斯帕尔马斯',

  // Italy - Serie B(意乙)
  'AC Monza': '蒙扎',
  'Calcio Padova': '帕多瓦',
  'Carrarese Calcio': '卡拉雷塞',
  'Cesena FC': '切塞纳',
  'Delfino Pescara': '佩斯卡拉',
  'Empoli FC': '恩波利',
  'FC Sudtirol Bolzano': '南蒂罗尔',
  'Frosinone Calcio': '弗罗西诺内',
  'Juve Stabia': '尤文斯塔比亚',
  'Mantova 1911': '曼托瓦',
  'Modena FC': '摩德纳',
  'Palermo FC': '巴勒莫',
  'Reggiana 1919': '雷吉亚纳',
  'SSC Bari': '巴里',
  'Sampdoria Genoa': '桑普多利亚',
  'Spezia Calcio': '斯佩齐亚',
  'US Avellino': '阿韦利诺',
  'US Catanzaro': '卡坦扎罗',
  'Venezia FC': '威尼斯',
  'Virtus Entella': '维尔图斯恩泰拉',

  // Germany - 2. Bundesliga(德乙)
  '1 FC Kaiserslautern': '凯泽斯劳滕',
  '1. FC Magdeburg': '马格德堡',
  'Arminia Bielefeld': '比勒费尔德',
  'Dynamo Dresden': '德累斯顿迪纳摩',
  'Eintracht Braunschweig': '布伦瑞克',
  'Fortuna Dusseldorf': '杜塞尔多夫',
  'Greuther Furth': '菲尔特',
  'Hannover 96': '汉诺威 96',
  'Hertha BSC': '柏林赫塔',
  'Holstein Kiel': '基尔',
  'Karlsruher SC': '卡尔斯鲁厄',
  'SC Paderborn 07': '帕德博恩',
  'SC Preussen 06 Munster': '普鲁士明斯特',
  'SV 07 Elversberg': '埃尔沃斯堡',
  'SV Darmstadt 98': '达姆施塔特',
  'VfL Bochum': '波鸿',

  // England - Championship(英冠)
  'Hull City': '赫尔城',
  'Middlesbrough FC': '米德尔斯堡',
  'Millwall FC': '米尔沃尔',
  'Southampton FC': '南安普顿',

  // Japan - J.League 2(日乙补)
  'AC Nagano Parceiro': '长野帕塞罗',
  'FC Gifu': '岐阜',
  'FC Imabari': '今治',
  'FC Osaka': '大阪 FC',
  'Matsumoto Yamaga FC': '松本山雅',
  'Renofa Yamaguchi': '山口雷诺法',
  'Tegevajaro Miyazaki': '宫崎太阳神',
  'Thespakusatsu': '草津温泉',
  'Tochigi City FC': '枥木城',

  // Republic of Korea - K-League 2(韩K2)
  'Chungbuk Cheongju FC': '忠北清州',
  'Chungnam Asan FC': '忠南牙山',
  'Daegu FC': '大邱 FC',
  'Gimpo FC': '金浦 FC',
  'Hwaseong FC': '华城 FC',
  'Seoul E-Land FC': '首尔伊蓝',
  'Suwon Bluewings': '水原三星',
  'Suwon FC': '水原 FC',

  // China - China League 1(中甲)
  'Changchun Yatai': '长春亚泰',
  'Jiangxi Dingnan United': '江西鼎南联',
  'Meizhou Hakka': '梅州客家',
  'Nanjing City': '南京城市',

  // Argentina - Liga Profesional(阿甲)
  'CA Independiente Avellaneda': '阿维拉内达独立',
  'CA River Plate (ARG)': '河床',
  'CA Rosario Central': '罗萨里奥中央',
  'CA San Lorenzo de Almagro': '圣洛伦索',
  'Racing Club Avellaneda': '阿维拉内达竞赛',

  // ── API-Football naming variants (added 2026-05-06) ─────────────────
  // Source data switched from odds-api.io → api-sports.io. Many teams have
  // slightly different canonical names. Mappings below are the upstream
  // names as returned by /fixtures, paired with the same Chinese strings
  // we already use elsewhere in this file.
  // Premier League (English-name canonicalized — drop "FC" / "AFC" suffix)
  'Liverpool': '利物浦',
  'Chelsea': '切尔西',
  'Brighton': '布莱顿',
  'Brentford': '布伦特福德',
  'Bournemouth': '伯恩茅斯',
  'Fulham': '富勒姆',
  'Wolves': '狼队',
  'Southampton': '南安普顿',
  // EFL Championship + lower
  'Middlesbrough': '米德尔斯堡',
  'Millwall': '米尔沃尔',
  'Sunderland': '桑德兰',
  // Spain
  'Sevilla': '塞维利亚',
  'Espanyol': '西班牙人',
  'Celta Vigo': '塞尔塔',
  'Osasuna': '奥萨苏纳',
  'Alaves': '阿拉维斯',
  'Levante': '莱万特',
  'Cadiz': '加的斯',
  'Almeria': '阿尔梅里亚',
  'Valladolid': '巴拉多利德',
  'Elche': '埃尔切',
  'Albacete': '阿尔巴塞特',
  'Zaragoza': '萨拉戈萨',
  'Burgos': '布尔戈斯',
  'Castellón': '卡斯特利翁',
  'Deportivo La Coruna': '拉科鲁尼亚',
  'AD Ceuta FC': '休达',
  // Italy
  'Inter': '国际米兰',
  'Lazio': '拉齐奥',
  'Torino': '都灵',
  'Udinese': '乌迪内斯',
  'Monza': '蒙扎',
  'Empoli': '恩波利',
  'Sassuolo': '萨索洛',
  'Cagliari': '卡利亚里',
  'Sampdoria': '桑普多利亚',
  'Frosinone': '弗罗西诺内',
  'Spezia': '斯佩齐亚',
  'Venezia': '威尼斯',
  'Bari': '巴里',
  'Palermo': '巴勒莫',
  'Cesena': '切塞纳',
  'Modena': '摩德纳',
  'Pescara': '佩斯卡拉',
  'Reggiana': '雷吉亚纳',
  'Catanzaro': '卡坦扎罗',
  'Avellino': '阿韦利诺',
  'Padova': '帕多瓦',
  'Mantova': '曼托瓦',
  'Sudtirol': '南蒂罗尔',
  'Carrarese': '卡拉雷塞',
  // Germany
  'Bayern München': '拜仁慕尼黑',
  'Borussia Mönchengladbach': '门兴格拉德巴赫',
  'VfL Wolfsburg': '沃尔夫斯堡',
  '1899 Hoffenheim': '霍芬海姆',
  '1. FC Kaiserslautern': '凯泽斯劳滕',
  // France
  'Paris Saint Germain': '巴黎圣日耳曼',
  'Lens': '朗斯',
  'Lille': '里尔',
  'Nantes': '南特',
  'Strasbourg': '斯特拉斯堡',
  // Netherlands
  'Hangzhou Greentown': '杭州绿城', // misclassified upstream; keeping for safety
  'Willem II': '威廉二世',
  'Waalwijk': '瓦尔韦克',
  // Scotland
  'Aberdeen': '阿伯丁',
  'Hibernian': '希伯尼安',
  'Kilmarnock': '基尔马诺克',
  'Dundee': '邓迪',
  'Dundee Utd': '邓迪联',
  'Dunfermline': '邓弗姆林竞技',
  'Falkirk': '福尔柯克',
  'Livingston': '利文斯顿',
  'ST Mirren': '圣米伦',
  'Arbroath': '阿布罗斯',
  // Belgium
  'OH Leuven': '奥哈瓦里',
  'Beerschot VA': '比尔肖特',
  'Lommel United': '洛梅尔',
  'RAAL La Louvière': '拉鲁维埃',
  'Dender': '丹德',
  'Zulte Waregem': '祖尔特瓦雷格姆',
  // Switzerland
  'Lausanne': '洛桑体育',
  'Grasshoppers': '草蜢',
  // Austria
  'Ried': '里德',
  'Grazer AK': '格拉茨AK',
  'WSG Wattens': '瓦滕斯',
  'FC BW Linz': '蓝白林茨',
  // Denmark
  'Sonderjyske': '南日德兰',
  'Viborg': '维堡',
  // Poland — note: Polish clubs are often named after the club word (Jagiellonia,
  // Raków, Widzew, Arka) NOT the city. Earlier I conflated club ↔ city; fixed.
  'Lech Poznan': '波兹南莱赫',
  'Lechia Gdansk': '格但斯克莱希亚',
  'Jagiellonia': '雅盖隆',
  'Raków Częstochowa': '拉科夫',
  'Widzew Łódź': '罗兹维泽夫',
  'Arka Gdynia': '阿尔卡格丁尼亚',
  // Greece
  'Atromitos': '阿特罗米托斯',
  'Larisa': '拉里萨',
  'Panetolikos': '帕涅托利科斯',
  'Panserraikos': '帕塞拉伊科斯',
  'Kifisia': '基菲西亚',
  // Russia / Ukraine — same club-vs-city pitfall as Poland: most names are
  // the club word (Polissya / Ruh / Epitsentr), not the city.
  'Dynamo Kyiv': '基辅迪纳摩',
  'Shakhtar Donetsk': '顿涅茨克矿工',
  'Zorya Luhansk': '卢甘斯克黎明',
  'Karpaty': '喀尔巴阡',
  'Kryvbas KR': '克里沃罗格克里夫巴斯',
  'Polessya': '日托米尔波利西亚',
  'Oleksandria': '亚历山德里亚',
  'Metalist 1925 Kharkiv': '哈尔科夫梅塔利斯特1925',
  'Ruh Lviv': '鲁赫',
  'Kudrivka': '库德里夫卡',
  'Epitsentr Dunayivtsi': '埃皮森特',
  // Turkey
  'Kasımpaşa': '卡塞帕萨',
  'Fatih Karagümrük': '卡拉古鲁库',
  'Gençlerbirliği S.K.': '青年会',
  // Japan (J-League).
  // Note: "Osaka" alone is ambiguous (could be Cerezo Osaka or Gamba Osaka)
  // so we don't map it here — let the original token through and rely on
  // upstream returning the disambiguated full name (e.g. "Cerezo Osaka").
  'Urawa': '浦和红钻',
  'Consadole Sapporo': '北海道札幌冈萨多',
  'Omiya Ardija': '大宫松鼠',
  'Thespakusatsu Gunma': '群马草津温泉',
  'Iwaki': '磐城',
  'V-varen Nagasaki': '长崎成功丸',
  'Imabari': 'FC今治',
  'Tochigi City': '枥木市',
  'Parceiro Nagano': '长野帕塞罗',
  // Korea
  'Pohang Steelers': '浦项制铁',
  'Jeju United FC': '济州联',
  'Daejeon Citizen': '大田市民',
  'Incheon United': '仁川联',
  'Gimpo Citizen': '金浦市民',
  'Hwaseong': '华城',
  'Cheongju': '忠北清州',
  'Asan Mugunghwa': '牙山木槿花',
  'Suwon City FC': '水原市民',
  // China
  'Shanghai Shenhua': '上海申花',
  'Henan Jianye': '河南建业',
  'Chengdu Better City': '成都蓉城',
  'Chongqing Tongliang Long': '重庆铜梁龙',
  'Heilongjiang Lava Spring': '黑龙江冰城',
  'Meizhou Kejia': '梅州客家',

  // ────────────────────────────────────────────────────────────────────
  // apifootball 短名别名(2026-05 起 fetcher 切到 apifootball,team 名
  // 变成短形式如 "Arsenal" 而不是 "Arsenal FC")。下面只列出**老 _zh 映射
  // 里没有**的短名 — 已重复的(如 'Liverpool''Chelsea''Sevilla''Inter'
  // 'Lazio''Bayern München''SC Freiburg''Lens''Strasbourg' 等)在前面 EPL/
  // LaLiga/Serie A/Bundesliga/Ligue1 段已经存在,此处不再重复。
  // ────────────────────────────────────────────────────────────────────
  // EPL 短名(尚未在前面段出现的)
  'Arsenal': '阿森纳',
  'Burnley': '伯恩利',
  'Everton': '埃弗顿',
  'Leeds': '利兹联',
  'Newcastle': '纽卡斯尔联',
  'Tottenham': '热刺',
  'West Ham': '西汉姆联',
  // LaLiga 短名
  'Athletic Club': '毕尔巴鄂竞技',
  'Barcelona': '巴塞罗那',
  'Getafe': '赫塔费',
  'Girona': '赫罗纳',
  'Mallorca': '马略卡',
  'Real Betis': '皇家贝蒂斯',
  'Oviedo': '皇家奥维耶多',
  'Real Sociedad': '皇家社会',
  'Valencia': '瓦伦西亚',
  'Villarreal': '比利亚雷亚尔',
  // Serie A 短名
  'Fiorentina': '佛罗伦萨',
  'Atalanta': '亚特兰大',
  'Bologna': '博洛尼亚',
  'Como': '科莫',
  'Genoa': '热那亚',
  'Juventus': '尤文图斯',
  'Parma': '帕尔马',
  'Pisa': '比萨',
  'Napoli': '那不勒斯',
  'Cremonese': '克雷莫内塞',
  'Lecce': '莱切',
  // Bundesliga(apifootball 短名/变音符)
  '1. FC Köln': '科隆',
  'FSV Mainz 05': '美因茨',
  // Ligue 1(老 _zh 没有/或者用了不同名)
  'Angers': '昂热',
  'Auxerre': '欧塞尔',
  'Le Havre': '勒阿弗尔',
  'Lorient': '洛里昂',
  'Lyon': '里昂',
  'Marseille': '马赛',
  'Metz': '梅斯',
  'Monaco': '摩纳哥',
  'Nice': '尼斯',
  'Rennes': '雷恩',
  'Stade Brestois 29': '布雷斯特',
  'Toulouse': '图卢兹',
  // ──────────────────────────────────────────────
  // 2026-05-11 懂球帝 batch translation — 251 teams
  // (auto-resolved via api.dongqiudi.com/search with hand
  //  curation for ambiguous + Saudi/Russian/Portuguese
  //  context-sensitive matches)
  // ──────────────────────────────────────────────
  '1. FC Nürnberg': '纽伦堡',
  'AEK Athens FC': '雅典AEK',
  'AIK Stockholm': 'AIK索尔纳',
  'AVS': '阿维什镇',
  'Aalesund': '奥勒松',
  'Aarhus': '奥胡斯',
  'Ajax': '阿贾克斯',
  'Akhmat': '格罗兹尼艾卡马特',
  'Akron': '陶里亚蒂阿克隆',
  'Al Khaleej Saihat': '赛哈特海湾',
  'Al Kholood': '拉斯永恒',
  'Al Najma': '纳吉马',
  'Al Okhdood': '阿科多',
  'Al Riyadh': '利雅得体育',
  'Al Shabab': '利雅得青年',
  'Al Taawon': '塔亚文',
  'Al-Ahli Jeddah': '吉达国民',
  'Al-Ettifaq': '达曼协作',
  'Al-Fateh': '哈萨征服',
  'Al-Fayha': '费哈',
  'Al-Hilal Saudi FC': '利雅得新月',
  'Al-Ittihad FC': '吉达联合',
  'Al-Nassr': '利雅得胜利',
  'Al-Qadisiyah FC': '卡迪斯亚',
  'Alverca': '阿维卡',
  'Amiens': '亚眠',
  'Anderlecht': '安德莱赫特',
  'Annecy': '阿讷西',
  'Ansan Greeners': '安山绿人',
  'Antwerp': '安特卫普',
  'Argentinos JRS': '阿根廷青年人',
  'Aris Thessalonikis': '阿里斯',
  'Arouca': '阿罗卡',
  'Atlas': '阿特拉斯',
  'Atletico Paranaense': '巴拉纳竞技',
  'Atletico-MG': '米内罗竞技',
  'Austin': '奥斯汀',
  'Austria Vienna': '奥地利维也纳',
  'Ayutthaya FC': '阿尤塔亚',
  'BSC Young Boys': '伯尔尼年轻人',
  'Bahia': '巴伊亚',
  'Baltika': '加里宁格勒波罗的海',
  'Bangkok United': '曼谷联合',
  'Bastia': '巴斯蒂亚',
  'Başakşehir': '巴萨克塞希尔',
  'Benfica': '本菲卡',
  'Beşiktaş': '贝西克塔斯',
  'Biwako Shiga': '琵琶湖滋贺',
  'Bodo/Glimt': '博德闪耀',
  'Botafogo': '博塔弗戈',
  'Boulogne': '博莱格尼',
  'Brann': '布兰',
  'Brondby': '布隆德比',
  'Busan I Park': '釜山IPark',
  'Casa Pia': '卡萨皮亚',
  'Celtic': '凯尔特人',
  'Chapecoense-sc': '沙佩科人',
  'Charleroi': '沙勒鲁瓦',
  'Charlotte': '夏洛特',
  'Cheonan City': '天安城',
  'Clermont Foot': '克莱蒙',
  'Club America': '阿美利加体育会',
  'Club Brugge KV': '布鲁日',
  'Cordoba': '科尔多瓦',
  'Corinthians': '科林蒂安',
  'Coritiba': '库里蒂巴',
  'Cracovia Krakow': '克拉科维亚',
  'Cruz Azul': '蓝十字',
  'Cruzeiro': '克鲁塞罗',
  'Dalian Huayi': '大连华艺',
  'Dalian Zhixing': '大连智行',
  'Damac': '达马克',
  'Dinamo Makhachkala': '马塔查克拉迪纳摩',
  'Dongguan United': '东莞联合',
  'Dunkerque': '敦刻尔克',
  'Dynamo': '莫斯科迪纳摩',
  'Ehime FC': '爱媛FC',
  'Eibar': '埃瓦尔',
  'Estac Troyes': '特鲁瓦',
  'Estoril': '埃斯托里尔',
  'Estrela': '阿马多拉之星',
  'Estudiantes L.P.': '拉普拉塔大学生',
  'Excelsior': 'SBV精英',
  'Eyüpspor': '埃于普体育',
  'FC Krasnodar': '克拉斯诺达尔',
  'FC Rostov': '罗斯托夫',
  'FC Ryukyu': 'FC琉球',
  'FC ST. Gallen': '圣加仑',
  'FC Schalke 04': '沙尔克04',
  'FC Sochi': '索契',
  'Famalicao': '法马利康',
  'Fenerbahçe': '费内巴切',
  'Feyenoord': '费耶诺德',
  'Flamengo': '弗拉门戈',
  'Fluminense': '弗鲁米嫩塞',
  'Fortuna Düsseldorf': '杜塞尔多夫',
  'Fredrikstad': '费德列斯达',
  'Fujieda MYFC': '藤枝MYFC',
  'Fukushima United': '福岛联合',
  'GIL Vicente': '吉尔维森特',
  'GO Ahead Eagles': '前进之鹰',
  'Gainare Tottori': '鸟取飞翔',
  'Gais': '加尔斯',
  'Galatasaray': '加拉塔萨雷',
  'Genk': '亨克',
  'Gent': '根特',
  'Gimhae City': '金海市民',
  'Gimnasia L.P.': '拉普拉塔体操',
  'Gremio': '格雷米奥',
  'Grenoble': '格勒诺布尔',
  'Groningen': '格罗宁根',
  'Guadalajara Chivas': 'CD瓜达拉哈拉',
  'Guangxi Hengchen': '广西恒宸',
  'Guangzhou E-Power': '广州豹',
  'Guimaraes': '吉马良斯',
  'Guingamp': '甘冈',
  'Gyeongnam FC': '庆南',
  'Göztepe': '哥兹塔比',
  'Halmstad': '哈尔姆斯塔德',
  'Ham-Kam': '汉坎',
  'Hammarby FF': '哈马比',
  'Heart Of Midlothian': '哈茨',
  'Hebei Kungfu': '河北功夫',
  'Heerenveen': '海伦芬',
  'Heracles': '阿尔梅罗大力神',
  'Huesca': '韦斯卡',
  'Independ. Rivadavia': '里瓦达维亚独立',
  'Inter Miami': '迈阿密国际',
  'Internacional': '巴西国际',
  'Jeonbuk Motors': '全北现代汽车',
  'Jeonnam Dragons': '全南天龙',
  'KV Mechelen': '梅赫伦',
  'Kagoshima United': '鹿儿岛联合',
  'Kamatamare Sanuki': '赞岐釜玉海',
  'Kanazawa': '金泽萨维根',
  'Kanchanaburi': '北碧巴吞旺龙',
  'Kashima': '鹿岛鹿角',
  'Kataller Toyama': '富山胜利',
  'Kitakyushu': '北九州向日葵',
  'Kochi United': '高知联合',
  'Krylia Sovetov': '萨马拉苏维埃之翼',
  'Kyoto Sanga': '京都不死鸟',
  'Lamphun Warrior': '南奔勇士',
  'Las Palmas': '拉斯帕尔马斯',
  'Lask Linz': '林茨',
  'Laval': '拉瓦勒',
  'Le Mans': '勒芒',
  'Leganes': '莱加内斯',
  'Levadiakos': '莱瓦贾科斯',
  'Lillestrom': '利勒斯特罗姆',
  'Lokomotiv': '莫斯科火车头',
  'Malaga': '马拉加',
  'Matsumoto Yamaga': '松本山雅',
  'Mirandes': '米兰德斯',
  'Mirassol': '米拉索尔',
  'Molde': '莫尔德',
  'Montpellier': '蒙彼利埃',
  'Moreirense': '莫雷拉人',
  'Motherwell': '马瑟韦尔',
  'Motor Lublin': '卢布林摩托',
  'Muangthong United': '孟通联合',
  'NEOM': '新未来城体育',
  'Nacional': '马德拉国民',
  'Nancy': '南锡',
  'Nara Club': '奈良',
  'Nieciecza': '涅切查',
  'Ningbo Professional': '宁波',
  'Nizhny Novgorod': '下诺夫哥罗德',
  'OFI': 'OFI克里特',
  'Obolon\'-Brovar': '奥布隆',
  'Odense': '欧登塞',
  'Olympiakos Piraeus': '奥林匹亚科斯',
  'Osaka': 'FC大阪',
  'PAOK': '塞萨洛尼基',
  'PAU': '波城',
  'Paju Citizen': '坡州市民',
  'Palmeiras': '帕尔梅拉斯',
  'Panathinaikos': '帕纳辛奈科斯',
  'Partick': '帕尔蒂克蓟花',
  'Piast Gliwice': '皮亚斯特',
  'Prachuap': '巴蜀基利',
  'Preußen Münster': '普鲁士明斯特',
  'Qingdao Jonoon': '青岛',
  'Qingdao Youth Island': '青岛青春岛',
  'RB Bragantino': '巴甘蒂诺PA',
  'RED Star FC 93': '红星',
  'Radomiak Radom': '拉多米亚克',
  'Rangers': '流浪者',
  'Rapid Vienna': '维也纳快速',
  'Ratchaburi': '拉查武里',
  'Real Sociedad II': '皇家社会B队',
  'Red Bull Salzburg': '萨尔茨堡红牛',
  'Reims': '兰斯',
  'Remo': '贝伦赛艇',
  'Rio Ave': '阿维河',
  'Rizespor': '里泽体育',
  'Rodez': '罗德兹',
  'Rosenborg': '罗森博格',
  'Rubin': '喀山红宝石',
  'SHANGHAI SIPG': '上海海港',
  'SK Poltava': 'SC波尔塔瓦',
  'SV Elversberg': '埃弗斯堡',
  'Sagamihara': 'SC相模原',
  'Saint Etienne': '圣埃蒂安',
  'San Diego': '圣地亚哥',
  'Sandefjord': '桑讷菲尤尔',
  'Santa Clara': '圣克拉拉',
  'Santos': '桑托斯',
  'Sao Paulo': '圣保罗',
  'Sarpsborg 08 FF': '萨普斯堡',
  'Seongnam FC': '城南',
  'Servette FC': '塞尔维特',
  'Shaanxi Union': '陕西联合',
  'Shandong Luneng': '山东泰山',
  'Shenyang Urban': '沈阳城市',
  'Shenzhen Juniors': '深圳青年人',
  'Shimizu S-pulse': '清水心跳',
  'Sichuan Jiuniu': '四川九牛',
  'Silkeborg': '锡尔克堡',
  'Sirius': '天狼星',
  'SpVgg Greuther Fürth': '菲尔特',
  'Spartak Moscow': '莫斯科斯巴达',
  'Sporting Gijon': '希洪体育',
  'St. Louis City': '圣路易斯城',
  'St. Truiden': '圣图尔登',
  'Start': '斯达',
  'Sturm Graz': '格拉茨风暴',
  'Telstar': '特尔斯达',
  'Tianjin Teda': '天津泰达',
  'Toluca': '托卢卡体育',
  'Tondela': '通德拉',
  'Tromso': '特罗姆瑟',
  'Twente': '特温特',
  'U.N.A.M. - Pumas': '美洲狮',
  'Ulsan Hyundai FC': '蔚山HD',
  'Union St. Gilloise': '圣吉罗斯联合',
  'Uthai Thani': '乌泰他尼',
  'Utrecht': '乌德勒支',
  'Valerenga': '瓦勒伦加',
  'Vancouver Whitecaps': '温哥华白浪',
  'Vanraure Hachinohe': '八户云罗里',
  'Vasco DA Gama': '瓦斯科达伽马',
  'Vasteras SK FK': '韦斯特罗斯',
  'Vejle': '瓦埃勒',
  'Viking': '维京',
  'Vitoria': '维多利亚体育',
  'Volos NFC': '沃洛斯',
  'Wuhan Three Towns': '武汉三镇',
  'Yokohama F. Marinos': '横滨水手',
  'Yongin City': '龙仁市民',
  'Zenit': '泽尼特',
};

/// 联赛名翻译(zh)。原数据格式: "England - Premier League"。
const Map<String, String> _zhLeague = {
  'England - Premier League': '英超',
  'England - Championship': '英冠',
  'Spain - LaLiga': '西甲',
  'Spain - LaLiga 2': '西乙',
  'Italy - Serie A': '意甲',
  'Italy - Serie B': '意乙',
  'Germany - Bundesliga': '德甲',
  'Germany - 2. Bundesliga': '德乙',
  'France - Ligue 1': '法甲',
  'France - Ligue 2': '法乙',
  'Netherlands - Eredivisie': '荷甲',
  'Portugal - Liga Portugal': '葡超',
  'Turkiye - Super Lig': '土超',
  'Brazil - Brasileiro Serie A': '巴甲',
  'Scotland - Premiership': '苏超',
  'Belgium - Pro League': '比甲',
  'Switzerland - Super League': '瑞超',
  'Austria - Bundesliga': '奥甲',
  'Denmark - Superliga': '丹麦超',
  'Sweden - Allsvenskan': '瑞典超',
  'Norway - Eliteserien': '挪威超',
  'Poland - Ekstraklasa': '波兰甲',
  'Greece - Super League': '希腊超',
  'Russia - Premier League': '俄超',
  'Ukraine - Premier League': '乌超',
  'Japan - J.League': '日职联',
  'Japan - J.League 2': '日乙',
  'Republic of Korea - K-League 1': '韩K1',
  'Republic of Korea - K-League 2': '韩K2',
  'China - Chinese Super League': '中超',
  'China - China League 1': '中甲',
  'Saudi Arabia - Saudi Pro League': '沙特联',
  'Thailand - Thai League 1': '泰超',
  'USA - MLS': '美职',
  'Mexico - Liga MX, Clausura': '墨西哥甲',
  'Argentina - Liga Profesional': '阿甲',
  'FIFA World Cup': '世界杯',
  'Fifa World Cup': '世界杯',
  'International Clubs - UEFA Champions League': '欧冠',
  'International Clubs - UEFA Europa League': '欧联',
  'International Clubs - UEFA Conference League': '欧协联',
  // 后端 prettifySlug fallback(没拉到 upstream 真名时)输出的格式 —
  // 把 slug 按 - 分段首字母大写,所以 "uefa" 变 "Uefa"、丢掉短横线。
  // 这些 entries 与上面的 upstream 真名各自命中一种场景。
  'International Clubs Uefa Champions League': '欧冠',
  'International Clubs Uefa Europa League': '欧联',
  'International Clubs Uefa Conference League': '欧协联',
  // ─── apifootball 真实上游显示名(部分带重音/与老 odds-api 标点不同) ───
  'Türkiye - Süper Lig': '土超',
  'Brazil - Brasileiro Série A': '巴甲',
  'Mexico - Liga MX Clausura': '墨西哥甲',
  'Japan - J League': '日职联',
  'Japan - J League 2': '日乙',
  'Republic of Korea - K League 1': '韩K1',
  'Republic of Korea - K League 2': '韩K2',
  // ─── 兜底:prettifySlug 输出(每个单词首字母大写,无连字符) ───
  // 当 ODDS_LEAGUES 配置了 slug 但上游还没返回任何 match,
  // 后端 fallback 走 prettifySlug 生成 chip 名,要保证仍能翻译。
  'Uefa Champions League': '欧冠',
  'Uefa Europa League': '欧联',
  'Uefa Conference League': '欧协联',
  'England Premier League': '英超',
  'England Championship': '英冠',
  'Spain Laliga': '西甲',
  'Spain Laliga 2': '西乙',
  'Italy Serie A': '意甲',
  'Italy Serie B': '意乙',
  'Germany Bundesliga': '德甲',
  'Germany 2 Bundesliga': '德乙',
  'France Ligue 1': '法甲',
  'France Ligue 2': '法乙',
  'Netherlands Eredivisie': '荷甲',
  'Portugal Liga Portugal': '葡超',
  'Turkiye Super Lig': '土超',
  'Scotland Premiership': '苏超',
  'Belgium Pro League': '比甲',
  'Switzerland Super League': '瑞超',
  'Austria Bundesliga': '奥甲',
  'Denmark Superliga': '丹超',
  'Sweden Allsvenskan': '瑞典超',
  'Norway Eliteserien': '挪超',
  'Poland Ekstraklasa': '波兰超',
  'Greece Super League': '希超',
  'Russia Premier League': '俄超',
  'Ukraine Premier League': '乌超',
  'Japan Jleague': 'J1 联赛',
  'Japan Jleague 2': 'J2 联赛',
  'Republic Of Korea K League 1': 'K1 联赛',
  'Republic Of Korea K League 2': 'K2 联赛',
  'China Chinese Super League': '中超',
  'China China League 1': '中甲',
  'Saudi Arabia Saudi Pro League': '沙特联',
  'Thailand Thai League 1': '泰超',
  'Brazil Brasileiro Serie A': '巴甲',
  'Usa Mls': '美职',
  'Mexico Liga Mx Clausura': '墨甲',
  'Argentina Liga Profesional': '阿甲',
};

// ─── Generated 14-language maps ──────────────────────────────────────────
// Source of truth is team_names_i18n.g.dart, produced by
// scripts/translate_teams.py from _zh / _zhLeague keys. Rerun the script
// after editing those maps.
const Map<String, Map<String, String>> _teamI18n = {
  'ru': gen.teamI18n_ru, 'es': gen.teamI18n_es,
  'ar': gen.teamI18n_ar, 'fa': gen.teamI18n_fa,
  'hi': gen.teamI18n_hi, 'id': gen.teamI18n_id,
  'ja': gen.teamI18n_ja, 'ko': gen.teamI18n_ko,
  'pt': gen.teamI18n_pt, 'tr': gen.teamI18n_tr,
  'vi': gen.teamI18n_vi, 'fr': gen.teamI18n_fr,
  'de': gen.teamI18n_de, 'it': gen.teamI18n_it,
};
const Map<String, Map<String, String>> _leagueI18n = {
  'ru': gen.leagueI18n_ru, 'es': gen.leagueI18n_es,
  'ar': gen.leagueI18n_ar, 'fa': gen.leagueI18n_fa,
  'hi': gen.leagueI18n_hi, 'id': gen.leagueI18n_id,
  'ja': gen.leagueI18n_ja, 'ko': gen.leagueI18n_ko,
  'pt': gen.leagueI18n_pt, 'tr': gen.leagueI18n_tr,
  'vi': gen.leagueI18n_vi, 'fr': gen.leagueI18n_fr,
  'de': gen.leagueI18n_de, 'it': gen.leagueI18n_it,
};

/// Localize a team name to the current locale. Resolution order:
///   1. admin override (zh / en columns only — runtime-editable)
///   2. hardcoded map for that locale
///   3. fallback to English original (admin en-override > raw upstream)
String localizedTeam(String original) {
  if (original.isEmpty) return original;
  final loc = I18n.instance.locale;
  if (loc == 'zh') {
    return TeamOverrides.instance.nameZh(original) ??
        _zh[original] ??
        original;
  }
  if (loc == 'en') {
    return TeamOverrides.instance.nameEn(original) ?? original;
  }
  final m = _teamI18n[loc];
  if (m != null) {
    final v = m[original];
    if (v != null && v.isNotEmpty) return v;
  }
  return TeamOverrides.instance.nameEn(original) ?? original;
}

/// 反向查找:把中文球队名翻成英文,用于把搜索框输入的中文映射回上游英文,
/// 后端匹配时仍然走 substring contains(home/away)。无映射 → 返回原 input。
///
/// 命中规则(从严到松):
///   1. 全等:"利物浦" → "Liverpool FC"
///   2. zh 名包含输入:"曼" → "曼联" → "Manchester United"
/// 如果输入没含中文字符,直接原样返回(显然是英文搜索)。
String resolveTeamSearchQuery(String input) {
  final trimmed = input.trim();
  if (trimmed.isEmpty) return trimmed;
  final hasChinese = RegExp(r'[一-鿿]').hasMatch(trimmed);
  if (!hasChinese) return trimmed;
  for (final e in _zh.entries) {
    if (e.value == trimmed) return e.key;
  }
  for (final e in _zh.entries) {
    if (e.value.contains(trimmed)) return e.key;
  }
  return trimmed;
}

/// Localize a league name. Same fallback behavior as [localizedTeam] but
/// without admin overrides (league table has no per-locale columns).
String localizedLeague(String original) {
  if (original.isEmpty) return original;
  final loc = I18n.instance.locale;
  if (loc == 'zh') return _zhLeague[original] ?? original;
  if (loc == 'en') return original;
  final m = _leagueI18n[loc];
  if (m != null) {
    final v = m[original];
    if (v != null && v.isNotEmpty) return v;
  }
  return original;
}
